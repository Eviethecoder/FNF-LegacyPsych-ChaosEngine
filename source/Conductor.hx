...
class Conductor
{
	...
	public static function mapBPMChanges(song:SwagSong)
	{
		...
		// collect BPM change events and sort by time
		var bpmEvents = song.events.filter(e -> e.name == "BPMChange");
		bpmEvents.sort((a, b) -> Std.int(a.strumTime - b.strumTime));

		var curBPM:Float = song.bpm;

		for (ev in bpmEvents)
		{
			// parse the new BPM from the first value
			var newBPM:Float = Std.parseFloat(Std.string(ev.values[0].value));
			if (newBPM <= 0 || newBPM == curBPM)
				continue;

			// derive step position from strumTime using accumulated map so far
			var stepTime:Float = getStep(ev.strumTime); // uses whatever is in bpmChangeMap so far

			bpmChangeMap.push({
				stepTime: Math.round(stepTime),
				songTime: ev.strumTime,
				bpm: newBPM,
		...
		// support both sections AND event bpm changes.
		var steps:Int = 0;
