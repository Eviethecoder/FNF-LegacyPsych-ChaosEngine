...
class DropShadowShader extends FlxShader
{
  ...
  /*
    Whether the shader should use the inverse of the alternate mask. (i.e. blue = light)
    False by default.
   */
  public var invertAltMask(default, set):Bool;
  ...
  public function loadAltMask(path:String)
  {
    ...
    #else
    var _bitmapData = BitmapData.fromFile(path);
    altMaskImage = _bitmapData;
    altMask.input = _bitmapData;
    trace('Loaded alt mask from ' + altMaskImage);
    trace('bitmap data is  ' + _bitmapData);
    #end
  }
  ...
  public function updateFrameInfo(frame:FlxFrame)
{
  ...
  function set_invertAltMask(val:Bool):Bool {
    invertAltMask = val;
    invertMask.value = [val];
    return val;
  }
  ...
  @:glFragmentSource('
      ...
      float intensityPass(vec2 fragCoord, float curThreshold, bool useMask) {
        ...
        if(useMask == true){
          float b = texture2D(altMask, fragCoord).b;
          if (invertMask) b = 1.0 - b;
          maskIntensity = mix(0.0, 1.0, b);
        }
