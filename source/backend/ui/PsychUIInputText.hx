...
class PsychUIInputText extends FlxSpriteGroup
{
				...
				case BACKSPACE:
					if(selectIndex < 0 || selectIndex == caretIndex)
					{
						...
						if(space > -1 && space != caretIndex-1)
						{
							...
							var newCaret:Int = caretIndex - Std.int(Math.max(0, text.length - (start.length + end.length)));
							@:bypassAccessor caretIndex = newCaret;
							text = start + end;
						}
						else
						{
							var oldCaret:Int = caretIndex;
							@:bypassAccessor caretIndex = 0;
							text = text.substring(oldCaret);
						}
						selectIndex = -1;
						if(onChange != null) onChange(lastText, text);
						if(broadcastInputTextEvent) PsychUIEventHandler.event(CHANGE_EVENT, this);
		...
		switch(flxKey)
		{
			...
			case BACKSPACE: //Delete letter to the left of caret
				...
				else
				{
					var lastText = text;
					var oldCaret:Int = caretIndex;
					var newCaret:Int = oldCaret - 1;
					@:bypassAccessor caretIndex = newCaret;
					text = text.substring(0, newCaret) + text.substring(oldCaret);
					if(onChange != null) onChange(lastText, text);
					if(broadcastInputTextEvent) PsychUIEventHandler.event(CHANGE_EVENT, this);
				}
				_nextAccent = NONE;
	...
	public function updateCaret()
	{
		...
		var maxIndex:Int = (text != null) ? text.length : 0;
		if(caretIndex < 0) @:bypassAccessor caretIndex = 0;
		else if(caretIndex > maxIndex) @:bypassAccessor caretIndex = maxIndex;
